...
export default function RouletteOption2Page() {
  ...
  const placeBet = useCallback((bt: BetType) => {
    ...
    if (!ok) { setMessage("Not enough chips!"); return; }
    setBets((prev) => {
      ...
      <div className="max-w-6xl mx-auto px-4 pt-6 pb-4">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-3xl md:text-4xl font-black tracking-tight">
            ...
            <span className="text-gray-500 text-sm ml-3 font-normal">Option 2: react-casino-roulette</span>
          </h1>
            ...
            {CHIP_VALUES.map((val) => (
              <button key={val} onClick={() => setSelectedChip(val)} disabled={spinning}
                className={`relative w-12 h-12 rounded-full font-bold text-xs flex items-center justify-center border-2 transition-all duration-150 ${
                  selectedChip === val ? "border-[var(--gold)] bg-[var(--gold)] text-black scale-110 shadow-lg shadow-[var(--gold)]/30" : "border-[var(--casino-border)] bg-[var(--casino-card)] text-gray-300 hover:border-gray-500"
            ...
            <div className="flex gap-[2px]">
              <div className="flex-shrink-0">
                <button onClick={() => placeBet({ kind: "straight", number: 0 })} disabled={spinning}
                  ...
                  0
                  {getBetAmount({ kind: "straight", number: 0 }) > 0 && <ChipBadge amount={getBetAmount({ kind: "straight", number: 0 })} />}
                </button>
              </div>
...
function OutsideBetButton({ label, betType, getBetAmount, placeBet, spinning, extraClass }: {
  label: string; betType: BetType; getBetAmount: (bt: BetType) => number; placeBet: (bt: BetType) => void; spinning: boolean; extraClass: string;
}) {
  const amt = getBetAmount(betType);
  return (
    <button onClick={() => placeBet(betType)} disabled={spinning}
